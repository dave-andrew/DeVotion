...
@section('content')
    <div class="w-full max-w-sm  px-6 mx-0 my-auto flex flex-col ">
        <div class="max-w-[380px] w-full my-8 flex flex-col text-xl text-left font-medium justify-center items-center">
            <h1 class="dark:text-white">Start your journey with <b class="text-2xl">DeVotion</b></h1>
        </div>
        <form action="{{ route('register') }}" method="POST" class="w-full flex flex-col text-grey-500">
            @csrf
            <label class="text-sm mb-2 dark:text-white" for="username">Username</label>
            <input class="auth-input dark:bg-gray-700 dark:text-white" type="text" name="username" id="username" placeholder="Enter your username..." value="{{old('username')}}">
            <label class="text-sm mb-2 dark:text-white" for="email">Email</label>
            <input class="auth-input dark:bg-gray-700 dark:text-white" type="text" name="email" id="email"
            ...
            <label class="text-sm mb-2 dark:text-white" for="password_confirmation">Confirm Password</label>
            <input class="auth-input dark:bg-gray-700 dark:text-white" type="password" name="password_confirmation" id="password_confirmation"
