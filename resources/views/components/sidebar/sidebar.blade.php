...
<div class="relative w-60 h-full min-h-screen flex flex-col bg-stone-100" x-data="{ search: false, setting: false, teamspace: false, workspace: false }" x-cloak>
    <div class="relative h-full max-h-full flex flex-col">
        ...
        <div id="workspaceDialogButton" class="h-8 sidebar-row my-2" onclick="openDialog()">
            ...
            <a href="{{ route('viewCreateWorkspace.type') }}" class="mr-1"><i
            ...
            @foreach ($workspace->teamspaces as $team)
                ...
                <div id="teamspace-notes-{{ $counter }}"  x-data="{ action: false }" x-cloak>
                    @foreach ($team->notes as $note)
                        ...
                        @include('components.sidebar.sidebar-workspace-action')
                    @endforeach
                </div>
            @endforeach
        </div>
